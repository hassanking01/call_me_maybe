...
class FSM:
    """Finite State Machine for constraining token generation."""
    ...
    def generate_number(self) -> None:
        """Add state transitions for float number parsing."""
        for c in "-0123456789":
            if c == "-":
    ...
    def generate_integer(
        ...
        """Add state transitions for integer parameter parsing.

        Args:
            idx: Index of parameter in function definition.
    ...
    def generate_str(self) -> None:
        """Add state transitions for string parameter parsing."""
        self.tier.setdefault(
            self.state,
            {}).update({'"': self.state + 1})
        self.tier.setdefault(
    ...
    def generate_boolean(self) -> None:
        """Add state transitions for boolean parameter parsing."""
    ...
    def create_tier(
        ...
        """Build state transitions from function definitions.
    ...
    def update_state(
        ...
        """Update current state based on generated token.

        Args:
            generated_token: Token string generated by model.
...
class Model:
    """LLM function calling model runner."""

    def __init__(self, parser: Parser):
        """Initialize model runner.

        Args:
            parser: Parser object containing configuration paths.
        """
    ...
    def validate_io_files(self) -> None:
        """Validate existence and path types of input/output files."""
        input_paths = [
    ...
    def get_vocabulary(self) -> None:
        """Load vocabulary mappings from model."""
    ...
    def collect_tokens(
        ...
        """Collect allowed token IDs starting from state.

        Args:
            state: Current state ID.
            token: Accumulated token string.
            allowed_tokens: Output list to append valid token IDs.
        """
        if state == -1:
            return
    ...
    def save_output(self) -> None:
        """Save formatted results to output JSON file."""
    ...
    def get_prompts(self) -> None:
        ...
        with open(self.parser.input) as f:
            ...
            try:
                ...
                raise ValueError(
                    "Missing required key 'prompt' in "
                    f" {self.parser.input}."
                )
    ...
    def get_valid_token(
            self,
            token: str) -> str:
        """Truncate token at closing quote if present.

        Args:
            token: Input token string.

        Returns:
            str: Truncated valid token string.
        """
        resutl = 0
        flag = True
    ...
    def get_allowed_tokens(
        ...
        """Get allowed token IDs for state.
    ...
    def mask_allow_tokens(
        ...
        """Mask logits based on allowed tokens.

        Args:
            state: Current state.
            logits: Unfiltered model output logits.

        Returns:
            list[float]: Masked logits list.
        """
    ...
    def run(self) -> None:
        """Run model inference on prompts and output function calls."""
        total = 0.0
